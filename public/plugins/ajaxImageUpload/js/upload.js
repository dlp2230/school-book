...
(function($){
    $.fn.ajaxImageUpload = function(options){
        ...
        var defaults = {
            ...
            maxNum: 10,
            hiddenInputName: '', // 上传成功后追加的隐藏input名，注意不要带[]，会自动带[]，不写默认和上传按钮的name相同
            maxSize: 50, //设置允许上传图片的最大尺寸，单位M
        ...
        var uploadBox = $(this).parent().parent().parent();
        ...
        var handleFileSelect = function(){
            ...
            // 获取最新的section数量-------原来的写法，有bug,不能再同一个页面使用多个上传框
            // var imageNum  = $('.image-section').length;


            // bug修复
            var imageNum  = $(thisObj).parents().parents().children('.image-section').length;
        ...
        var ajaxUpload = function () {
            ...
            $.ajax({
                ...
                success:function(json){
                    ...
                    var imageShow = imageSection.children('img');
        ...
        var createImageSection = function () {
            ...
            var hiddenInputName = config.hiddenInputName;
            ...
            var imageInput   = $("<input class='" + inputName + "' name='" + hiddenInputName + "[]' value='' type='hidden'>");
